...
export function AuthScreen({ onSuccess }: { onSuccess: (user: LocalUser) => void }) {
  ...
  const submit = (event: FormEvent) => {
    ...
    if (mode === 'register') {
      if (password.length < 6) return setError('Mật khẩu phải có tối thiểu 6 ký tự.')
